...
print('Running model {} in code reviews'.format(m_name))

#### Read data
train_data=pd.read_pickle(data_folder/'cr-train.pkl')
train_data['label']=train_data['label'].replace(-1, 1)

X_train=train_data['sentence']
y_train=train_data['label']

test_data=pd.read_pickle(data_folder/'cr-test.pkl')
test_data['label']=test_data['label'].replace(-1, 1)
...
if rerun_flag:
    ...
    to_save={'cr_{}'.format(m_name): model}
    ...
    run_saved_model(prediction_dataloader, cur_model, 'cr', m_name)