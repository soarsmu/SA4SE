...
train_df=pd.read_pickle(so_train)
...
print('Training used {:.2f} second'.format(end-begin))
...
test_end=time.time()
print('Prediction used {:.2f} second'.format(test_end-test_begin))

flat_predictions = [item for sublist in predictions for item in sublist]
flat_predictions = np.argmax(flat_predictions, axis=1).flatten()
flat_true_labels = [item for sublist in true_labels for item in sublist]

print("Accuracy of {} on Stack Overflow is: {}".format(m_name, accuracy_score(flat_true_labels,flat_predictions)))
