...
print('Running model {} in Jira'.format(m_name))

#### Read data
train_data=pd.read_pickle(data_folder/'jira-train.pkl')
train_data['label']=train_data['label'].replace(-1, 0)

X_train=train_data['sentence']
y_train=train_data['label']

test_data=pd.read_pickle(data_folder/'jira-test.pkl')
...
if rerun_flag:
    ...
    to_save={'jira_{}'.format(m_name): model}
    ...
    run_saved_model(prediction_dataloader, cur_model, 'jira', m_name)