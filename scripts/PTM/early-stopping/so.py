...
print('Running model {} in Stack Overflow'.format(m_name))

#### Read data
train_data=pd.read_pickle(data_folder/'so-train.pkl')
train_data['label']=train_data['label'].replace(-1, 2)

X_train=train_data['sentence']
y_train=train_data['label']

test_data=pd.read_pickle(data_folder/'so-test.pkl')
...
if rerun_flag:
    ...
    to_save={'so_{}'.format(m_name): model}
    ...
    run_saved_model(prediction_dataloader, cur_model, 'so', m_name)